/**
 * One-off script: create minimal screen handlers for all Performance menu screens
 * that don't already have a handler. Reads Webinfra.Server/AppMenus/HSE.json,
 * finds Performance subtree, writes handler files and prints index.js additions.
 * Skips Drill Plan Entry (HSE_DrllPlnEntryAtEntry) - already implemented.
 */
const fs = require('fs');
...
function getPerformanceNode(menu) {
  const hse = menu?.children?.[0]?.children;
  if (!Array.isArray(hse)) return null;
  return hse.find((n) => n.English_Caption === 'Performance') || null;
}
...
function main() {
  ...
  const perfNode = getPerformanceNode(menu.Menu);
  if (!perfNode) {
    console.error('Performance node not found in HSE.json');
    process.exit(1);
  }
  const screens = [];
  collectScreensWithTag(perfNode, [], screens, true);

  const toAdd = screens.filter((s) => !EXISTING_TAGS.has(s.tag));
  console.log(`Found ${screens.length} Performance screens, ${toAdd.length} need handlers (${screens.length - toAdd.length} already implemented).`);

  const imports = [];
  const entries = [];

  for (const { tag, pathSegments, caption } of toAdd) {
    const relFolder = pathSegments.length ? 'Performance/' + pathSegments.join('/') : 'Performance';
    const fileName = captionToFileName(caption);
    const dirPath = path.join(SCREEN_HANDLERS_ROOT, relFolder);
    const filePath = path.join(dirPath, fileName);
    const menuPath = 'Performance -> ' + (pathSegments.length ? pathSegments.join(' -> ') + ' -> ' : '') + caption;
