/**
 * One-off script: create minimal screen handlers for all Environment menu screens
 * that don't already have a handler. Reads Webinfra.Server/AppMenus/HSE.json,
 * finds Environment subtree, writes handler files and prints index.js additions.
 */
const fs = require('fs');
...
function getEnvironmentNode(menu) {
  const hse = menu?.children?.[0]?.children;
  if (!Array.isArray(hse)) return null;
  return hse.find((n) => n.English_Caption === 'Environment') || null;
}
...
function main() {
  ...
  const envNode = getEnvironmentNode(menu.Menu);
  if (!envNode) {
    console.error('Environment node not found in HSE.json');
    process.exit(1);
  }
  const screens = [];
  collectScreensWithTag(envNode, [], screens, true);

  const toAdd = screens.filter((s) => !EXISTING_TAGS.has(s.tag));
  console.log(`Found ${screens.length} Environment screens, ${toAdd.length} need handlers.`);

  const imports = [];
  const entries = [];

  for (const { tag, pathSegments, caption } of toAdd) {
    const relFolder = 'Environment/' + pathSegments.join('/');
    const fileName = captionToFileName(caption);
    const dirPath = path.join(SCREEN_HANDLERS_ROOT, relFolder);
    const filePath = path.join(dirPath, fileName);
    const menuPath = 'Environment -> ' + (pathSegments.length ? pathSegments.join(' -> ') + ' -> ' : '') + caption;
