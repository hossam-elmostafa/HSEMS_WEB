 ...
 */
/*
--The below code is the code from github before resolving a conflict
async function getUserDepartment(devInterface) {
  const { getValFromRecordSet, getUserName, getCurrentUserObj } = devInterface || {};
  const executeSQLPromise = devInterfaceObj?.executeSQLPromise || devInterfaceObj?.executeSQL;
...
export async function onMenuItemClicked(userObj, strScrTag, callBackFn) {
  ...
  if (userDept && ownerDept && userDept !== ownerDept) {
    strScrCriteria = `WHERE CRENTRY_DPR='${escSql(userDept)}' or CRENTRY_CNCDPR='${escSql(userDept)}'`;//RQ_HSM_29_6_26_13_5
  }
